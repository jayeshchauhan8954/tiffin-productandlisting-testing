...
import { ButtonBase, Grid, Stack, TextField, Typography, useTheme } from '@mui/material'
import React, { useRef, useState } from 'react'

export default function VerifyOTP({ email, phone, dialCode, onVerifyOTP = () => { } }) {
    ...
    const handleVerifyOTP = async () => {
        ...
        const { status } = await apiRequest({
            method: 'PUT',
            endUrl: _apiUrls.auth.verifyOtpAuth,
            body: { type: phone ? _otpTypes.sms : _otpTypes.email, otp: otp?.join(""), phone, email },
            showMsg: true
        })
        setLoad(false)
        if (status) {
            ...
            <Grid container>
                <Grid xs={12} md={12}>
                    <Stack sx={{ alignItems: 'center' }}>
                        ...
                        <Typography variant='cardText' color={theme.palette.primary.primaryDmain2}>{Boolean(phone) ? `${dialCode} ${phone}` : `${email}`}</Typography>
                    </Stack>
                </Grid>
                ...
                <Grid xs={12} md={12}>
                    <Stack sx={{ textAlign: 'center' }}>
                        <ButtonBase onClick={handleResendOTP}>
                            <Typography color={theme.palette.primary.main} sx={{ textDecorationLine: 'underline' }} fontSize={16} variant='small1' >Resend Code</Typography>
                        </ButtonBase>
                    </Stack>
                </Grid>
                ...
                <Grid xs={12} md={12}>
                    ...
                    >
                        Verify {email ? "email" : "phone number"}
                    </LoadingButton>
                </Grid>
            </Grid>
        </>
